# Spirit on iPad — Design round 2 (gap pass, paste-ready)

Surgical follow-up to the five iPad screens already in the Claude Design
project ("Pitaya iPad 00 - Home" … "04 - Guided Study"). Paste everything
between the markers. What it addresses: the main lane's review of the
round-1 hand-back (`docs/spirit-ipad-build-round1-from-design.md`, review
notes at the top) plus three things Michael added on 2026-08-22 after
seeing round 1: **the Bible ink overlay** ("my notes on the Bible — freely
scribble on it"; round 1's Scratch mode showed marks, not a notes layer),
**Home as a mini-hub** (ratified: Spirit is the iPad's front door, with
3–4 glance widgets and the right-hand rail to the other sections), and the
**export ask** — the repo's build tool can only read design-system-type
projects, so the screens must come back as downloadable `.dc.html` files
(or the canvas moves into a design-system project).

**After this round:** he drops the files (round 1's five + round 2's) into
`docs/design/`, and the main lane writes the build kickoff — data spine,
native list, merge preflight — and runs the iPad sequence under the PORT
GATE, Sunday slice first. Clean copy: `docs/spirit-ipad-design-prompt-round2-PASTE.md`.

# Spirit on iPad — Claude Design prompt (round 1, paste-ready)

Companion to `docs/spirit-ipad-brainstorm.md` (the brainstorm, and in its
§15 **his answers of 2026-08-22**) and `docs/spirit-journal-plan.md` (the
thesis — still the law). This file is the **prompt itself** — paste
everything between the markers into Claude Design, then iterate. Clean
copy without this header: `docs/spirit-ipad-design-prompt-PASTE.md`.

**What his answer sheet decided, so the prompt doesn't drift:** native
ink first (Apple Pencil Pro on an 11" iPad Air; the notebook pane will be
PencilKit-backed inside the iPad companion, the Reader stays the web
Reader) · **landscape-first**, portrait stacked · **left-handed default
with a handedness setting** (he may share the app one day) · a **pane
system like Logos** — two or three panes, **two Bibles allowed** (a main
text and a reference Bible that follows links) · the Bible pane in **two
modes** — Study (marks evaporate, hover) and Scratch (his ink stays on a
frozen, printed-style page) · **the highlighter tool is what highlights;
an underline is an underline** · action bar: **design two options**
(pen-positioned vs. fixed upper-right) · **free palette** with recents +
saved palettes, **brush styles** (fountain pen, G-pen, pencil, marker),
**multiple notebooks** · handwriting stays handwriting, hidden text
layer, references he writes become live links with a Logos-style
popover · **Sunday is the pilot**: sermon page + recording in the corner
+ tap-a-stroke replay + a **recordings library** · **every study gets a
written assignment**; worksheets are form-like with a **Submit** button,
never auto-ticked · AI never responds to his written answers ·
**Journal deferred (maybe scrapped), Health on iPad is round 2.**

**Build kickoff for the next chat** comes after the design lands (PORT
GATE): see `docs/spirit-ipad-brainstorm.md` §15 for the build-facing
decisions (C-first, the $99 program, one chat owning web + ios for this
project, the curriculum change, recordings storage).
